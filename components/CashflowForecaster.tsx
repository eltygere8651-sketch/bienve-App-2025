            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-800/40 p-4 rounded-xl border border-slate-700/50">
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto">
                    ...
                    <input 
                        ...
                        className="bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-blue-500 transition-colors w-full sm:w-32"
                    />
                </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-slate-800/60 border border-slate-700/50 p-5 rounded-2xl relative overflow-hidden group">
                    <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:scale-110 transition-transform">
                        <Activity size={60} className="text-blue-400" />
                    </div>
                    <p className="text-slate-400 font-bold text-xs uppercase mb-1">Tesorería Actual</p>
                    <h3 className="text-2xl font-black text-white">{formatCurrency(treasuryCurrent)}</h3>
                    <p className="text-[10px] text-slate-500 mt-1">Disponible líquido</p>
                </div>
                ...
                <div className="bg-slate-800/60 border border-slate-700/50 p-5 rounded-2xl relative overflow-hidden group">
                    ...
                    <p className="text-slate-400 font-bold text-xs uppercase mb-1">Ingreso Proyectado ({monthsToProject}M)</p>
                    <h3 className="text-2xl font-black text-emerald-400">+{formatCurrency(totalProjected)}</h3>
                    <p className="text-[10px] text-slate-500 mt-1">Ajustado por índice de riesgo</p>
                </div>
                
                <div className="bg-slate-800/60 border border-primary-500/30 bg-primary-900/10 p-5 rounded-2xl relative overflow-hidden group">
                    <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:scale-110 transition-transform">
                        <BarChart3 size={60} className="text-primary-400" />
                    </div>
                    <p className="text-primary-300 font-bold text-xs uppercase mb-1">Caja Estimada en {monthsToProject}M</p>
                    <h3 className="text-2xl font-black text-primary-400">{formatCurrency(futureTreasury)}</h3>
                    <p className="text-[10px] text-primary-500/70 mt-1">Al cumplir la proyección</p>
                </div>
            </div>
                ...
                <div className="flex justify-between items-center mb-6">
                    <div>
                        <h3 className="text-lg font-bold text-white flex items-center gap-2">
                            ...
                            Flujo de Caja Futuro ({monthsToProject} Meses)
                        </h3>
                        <p className="text-xs text-slate-400">Basado en cronogramas activos y ajustado a la morosidad actual</p>
                    </div>
                    <div className="bg-amber-500/10 text-amber-500 border border-amber-500/20 px-3 py-1.5 rounded-lg flex items-center gap-2 text-xs font-bold">
