...
import { Handshake, LogIn, Mail, Key, Loader2 } from 'lucide-react';
import { useAppContext } from '../contexts/AppContext';
import { signIn } from '../services/supabaseService';

const REMEMBER_ME_KEY = 'bmContigoRememberEmail';
...
const Auth: React.FC = () => {
    ...
    const handleLoginSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // onAuthStateChanged en AppContext se encargará de la redirección
        } catch (err: any) {
                    ...
                    <form onSubmit={handleLoginSubmit} className="space-y-6">
                        <div>
                            ...
                            <div className="relative mt-1">
                                ...
                                <input id="auth-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="pl-10 w-full px-4 py-2 border border-slate-600 rounded-md shadow-sm focus:ring-primary-500 focus:border-primary-500 bg-slate-700 text-slate-100" />
                            </div>
                        </div>
                        <div>
                            ...
                            <div className="relative mt-1">
                                ...
                                <input id="auth-password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} required className="pl-10 w-full px-4 py-2 border border-slate-600 rounded-md shadow-sm focus:ring-primary-500 focus:border-primary-500 bg-slate-700 text-slate-100" />
                            </div>
                        </div>
                        ...
                        <div className="flex items-center">
                            <input
                                ...
                                className="h-4 w-4 text-primary-600 border-slate-500 bg-slate-700 focus:ring-primary-500 rounded"
                            />
                        ...
                        <div className="pt-2">
                            <button type="submit" disabled={isLoading} className="w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 disabled:bg-primary-400">
                                {isLoading ? <Loader2 className="animate-spin h-5 w-5" /> : <><LogIn size={18} className="mr-2"/> Iniciar Sesión</>}
                            </button>
                        </div>
                    </form>
                </div>
