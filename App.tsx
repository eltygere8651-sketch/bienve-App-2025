             ...
             <div className="p-5 border-t border-white/5 bg-black/10 flex-shrink-0 safe-area-bottom">
                ...
                <div className={`grid ${isSidebarOpen || isMobileMenuOpen ? 'grid-cols-2' : 'grid-cols-1'} gap-3 mb-4`}>
                     {!isInStandaloneMode && (
                        <button 
                            onClick={handleInstallApp}
                            className="flex flex-col items-center justify-center p-3 rounded-xl transition-all border group bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white border-white/5 shadow-sm hover:shadow-lg hover:-translate-y-1"
                            title="Instalar App"
                        >
                    ...
                    <button 
                        onClick={() => setIsShareModalOpen(true)}
                        className={`flex flex-col items-center justify-center p-3 rounded-xl transition-all border group bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white border-white/5 shadow-sm hover:shadow-lg hover:-translate-y-1 ${isInStandaloneMode ? 'col-span-1' : ''}`}
                        title="Compartir App"
                    >
            ...
            */}
            <div className="fixed inset-0 flex bg-slate-900 font-sans text-slate-300 overflow-hidden">
                ...
                <div className={`md:hidden fixed inset-0 z-50 transition-transform duration-300 ease-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                 ...
                 {/* Desktop Sidebar */}
                 <aside className={`hidden md:flex transition-all duration-300 ease-in-out border-r bg-slate-900/40 backdrop-blur-xl flex-col ${isSidebarOpen ? 'w-72' : 'w-20'} border-white/5`}>
                    <SidebarContent />
                </aside>
                ...
                <main className="flex-1 flex flex-col h-full relative w-full">
                    ...
                    <div className="absolute top-0 left-0 w-full h-96 transition-colors rounded-full blur-[120px] -translate-y-1/2 pointer-events-none bg-primary-900/10"></div>
